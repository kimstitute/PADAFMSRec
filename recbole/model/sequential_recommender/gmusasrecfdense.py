...
PADAF-compatible dense-cache baseline that uses a Gated Multimodal Unit (GMU)
for early fusion before the standard SASRec Transformer encoder. Item id,
structured side features, and precomputed dense item features are projected to a
shared hidden space; a learned softmax gate then computes a per-position
weighted sum of those branches.
"""
...
class GMUSASRecFDense(SequentialRecommender):
    """SASRec with GMU early fusion over structured and dense cache features."""

    def __init__(self, config, dataset):
        super(GMUSASRecFDense, self).__init__(config, dataset)
        ...
        self.num_gmu_branches = 1 + self.num_feature_field
        ...
        self.branch_transforms = nn.ModuleList(
            [nn.Linear(self.hidden_size, self.hidden_size) for _ in range(self.num_gmu_branches)]
        )
        self.gate_layer = nn.Linear(
            self.hidden_size * self.num_gmu_branches, self.num_gmu_branches
        )
    ...
    def _validate_config(self, config):
        if not self.selected_features:
            raise ValueError("GMUSASRecFDense requires at least one selected feature.")
        if self.selected_features != self.feature_names:
            raise ValueError(
                "selected_features must equal structured_features + dense_features "
                "to keep GMUSASRecFDense branch order explicit. got "
        ...
        if len(set(self.feature_names)) != len(self.feature_names):
            raise ValueError("GMUSASRecFDense feature names must be unique.")
    ...
    def _gmu_fuse(self, branch_tensors):
        transformed = [
            torch.tanh(transform(branch_tensor))
            for transform, branch_tensor in zip(self.branch_transforms, branch_tensors)
        ]
        gate_input = torch.cat(branch_tensors, dim=-1)
        gate = torch.softmax(self.gate_layer(gate_input), dim=-1)
        stacked = torch.stack(transformed, dim=-2)
        return torch.sum(gate.unsqueeze(-1) * stacked, dim=-2)

    def forward(self, item_seq, item_seq_len, return_gates=False):
        ...
        branch_tensors = [item_emb] + self._get_feature_tensors(item_seq)
        input_emb = self._gmu_fuse(branch_tensors)
        ...
        if return_gates:
            ...
            return {
                ...
                "gates": torch.softmax(self.gate_layer(gate_input), dim=-1),
            }
        return seq_output
